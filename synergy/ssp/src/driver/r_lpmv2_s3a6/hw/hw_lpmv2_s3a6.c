...
/**********************************************************************************************************************
 * File Name    : hw_lpmv2_s3a6.c
 * Description  : LPMV2 S3A6 hardware implementation.
 **********************************************************************************************************************/
...
#if defined(BSP_MCU_GROUP_S3A6)

#include "r_lpmv2_s3a6.h"
#include "hw_lpmv2_s3a6.h"
#include "r_cgc.h"
...
ssp_err_t HW_LPMV2_MCUSpecificLowPowerCheck(lpmv2_cfg_t const * const p_cfg)
{
    ...
    if(LPMV2_LOW_POWER_MODE_SLEEP != p_cfg->low_power_mode)
    {
        ...
        lpmv2_mcu_cfg_t * p_lpmv2_s3a6_cfg = (lpmv2_mcu_cfg_t * )(p_cfg->p_extend);

        if(LPMV2_LOW_POWER_MODE_STANDBY_SNOOZE == p_cfg->low_power_mode)
        {
            ssp_err_t err = check_snooze_config(p_lpmv2_s3a6_cfg->dtc_state_in_snooze,
                                                p_lpmv2_s3a6_cfg->snooze_request_source,
                                                p_lpmv2_s3a6_cfg->snooze_end_sources);
            LPMV2_ERROR_RETURN(SSP_SUCCESS == err, err);
        }

        LPMV2_ERROR_RETURN(0UL == ((uint32_t)p_lpmv2_s3a6_cfg->standby_wake_sources & ~HW_LPMV2_STANDBY_WAKE_SOURCE_MASK), SSP_ERR_INVALID_MODE);
...
void HW_LPMV2_MCUSpecificConfigure(lpmv2_cfg_t const * const p_cfg)
{
       ...
       (NULL != p_cfg->p_extend))
    {
        ...
        HW_LPMV2_WUPENSet(p_lpmv2_s3a6_cfg->standby_wake_sources);

        /* OPE is reserved on S3A6. Write value should be 1 */
        R_SYSTEM->SBYCR_b.OPE = 1U;

        if(LPMV2_LOW_POWER_MODE_STANDBY_SNOOZE == p_cfg->low_power_mode)
        {
            HW_LPMV2_SnoozeConfigure(p_lpmv2_s3a6_cfg->dtc_state_in_snooze,
                                     p_lpmv2_s3a6_cfg->snooze_request_source,
                                     p_lpmv2_s3a6_cfg->snooze_end_sources,
                                     p_lpmv2_s3a6_cfg->snooze_cancel_sources);
