...
/**********************************************************************************************************************
 * File Name    : hw_lpmv2_s5d5.c
 * Description  : LPMV2 S5D5 hardware implementation.
 **********************************************************************************************************************/
...
#include "r_lpmv2_s5d5.h"
#include "hw_lpmv2_s5d5.h"
#include "r_cgc.h"
...
static ssp_err_t check_deep_standby_config(lpmv2_mcu_cfg_t * p_lpmv2_s5d5_cfg);
static ssp_err_t check_standby_config(lpmv2_mcu_cfg_t * p_lpmv2_s5d5_cfg);
...
ssp_err_t HW_LPMV2_MCUSpecificLowPowerCheck(lpmv2_cfg_t const * const p_cfg)
{
    ...
    if(LPMV2_LOW_POWER_MODE_SLEEP != p_cfg->low_power_mode)
    {
        ...
        lpmv2_mcu_cfg_t * p_lpmv2_s5d5_cfg = (lpmv2_mcu_cfg_t * )(p_cfg->p_extend);

        if(LPMV2_LOW_POWER_MODE_STANDBY_SNOOZE == p_cfg->low_power_mode)
        {
            err = check_snooze_config(p_lpmv2_s5d5_cfg->dtc_state_in_snooze,
                                                p_lpmv2_s5d5_cfg->snooze_request_source);
            LPMV2_ERROR_RETURN(SSP_SUCCESS == err, err);
        }
        else if(LPMV2_LOW_POWER_MODE_DEEP == p_cfg->low_power_mode)
        {
            err = check_deep_standby_config(p_lpmv2_s5d5_cfg);
            LPMV2_ERROR_RETURN(SSP_SUCCESS == err, err);
        }

        err = check_standby_config(p_lpmv2_s5d5_cfg);
        LPMV2_ERROR_RETURN(SSP_SUCCESS == err, err);
    }
 ...
 *
 * @param p_lpmv2_s5d5_cfg the MCU specific configuration
 *
 * @return SSP_SUCCESS              configuration is valid
 * @return SSP_ERR_INVALID_MODE     invalid power supply option for deep standby
 * @return SSP_ERR_INVALID_MODE     invalid IO port option for deep standby
...
static ssp_err_t check_deep_standby_config(lpmv2_mcu_cfg_t * p_lpmv2_s5d5_cfg)
{
    ...
    LPMV2_ERROR_RETURN(true ==
        check_deep_standby_cancel_sources(p_lpmv2_s5d5_cfg->deep_standby_cancel_source),
        SSP_ERR_INVALID_MODE);

    LPMV2_ERROR_RETURN(true ==
        check_deep_standby_cancel_edges(p_lpmv2_s5d5_cfg->deep_standby_cancel_edge),
        SSP_ERR_INVALID_MODE);

    LPMV2_ERROR_RETURN(true == check_deep_standby_cancel_sources_edges(
                            p_lpmv2_s5d5_cfg->deep_standby_cancel_source,
                            p_lpmv2_s5d5_cfg->deep_standby_cancel_edge),
                       SSP_ERR_INVALID_MODE);

    err = check_power_supply_state(p_lpmv2_s5d5_cfg->power_supply_state);
    LPMV2_ERROR_RETURN(SSP_SUCCESS == err, err);

    err = check_io_port_state(p_lpmv2_s5d5_cfg->io_port_state);
    LPMV2_ERROR_RETURN(SSP_SUCCESS == err, err);
...
static ssp_err_t check_standby_config(lpmv2_mcu_cfg_t * p_lpmv2_s5d5_cfg)
{
    LPMV2_ERROR_RETURN(true == check_wake_sources(p_lpmv2_s5d5_cfg->standby_wake_sources), SSP_ERR_INVALID_MODE);

    LPMV2_ERROR_RETURN(true == check_output_port_config(p_lpmv2_s5d5_cfg->output_port_enable), SSP_ERR_INVALID_MODE);
...
void HW_LPMV2_MCUSpecificConfigure(lpmv2_cfg_t const * const p_cfg)
{
       ...
       (NULL != p_cfg->p_extend))
    {
        ...
        if(LPMV2_LOW_POWER_MODE_DEEP == p_cfg->low_power_mode)
        {
            ...
            R_SYSTEM->DSPBYCR_b.DEEPCUT = DPSBYCR_DEEPCUT_MASK & p_lpmv2_s5d5_cfg->power_supply_state;

            HW_LPMV2_DPSIER0Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d5_cfg->deep_standby_cancel_source       )));
            HW_LPMV2_DPSIER1Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d5_cfg->deep_standby_cancel_source >> 8U )));
            HW_LPMV2_DPSIER2Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d5_cfg->deep_standby_cancel_source >> 16U)));
            HW_LPMV2_DPSIER3Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d5_cfg->deep_standby_cancel_source >> 24U)));

            HW_LPMV2_DPSIEGR0Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d5_cfg->deep_standby_cancel_edge       )));
            HW_LPMV2_DPSIEGR1Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d5_cfg->deep_standby_cancel_edge >> 8U )));
            HW_LPMV2_DPSIEGR2Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d5_cfg->deep_standby_cancel_edge >> 16U)));

            R_SYSTEM->DSPBYCR_b.IOKEEP = DPSBYCR_IOKEEP_MASK & p_lpmv2_s5d5_cfg->io_port_state;
        }
        else if(LPMV2_LOW_POWER_MODE_STANDBY_SNOOZE == p_cfg->low_power_mode)
        {
            HW_LPMV2_SnoozeConfigure(p_lpmv2_s5d5_cfg->dtc_state_in_snooze,
                                     p_lpmv2_s5d5_cfg->snooze_request_source,
                                     p_lpmv2_s5d5_cfg->snooze_end_sources,
                                     p_lpmv2_s5d5_cfg->snooze_cancel_sources);
            R_SYSTEM->SNZCR |= SNZCR_SNZE_MASK;
        }

        R_SYSTEM->SBYCR_b.OPE = SBYCR_OPE_MASK & p_lpmv2_s5d5_cfg->output_port_enable;

        HW_LPMV2_WUPENSet(p_lpmv2_s5d5_cfg->standby_wake_sources);
