/***********************************************************************************************************************
 * Copyright [2016-2024] Renesas Electronics Corporation and/or its licensors. All Rights Reserved.
 * 
...
/**********************************************************************************************************************
 * File Name    : hw_lpmv2_s5d3.c
 * Description  : LPMV2 S5D3 hardware implementation.
 **********************************************************************************************************************/
...
#if defined(BSP_MCU_GROUP_S5D3)

#include "r_lpmv2_s5d3.h"
#include "hw_lpmv2_s5d3.h"
#include "r_cgc.h"
...
static ssp_err_t check_deep_standby_config(lpmv2_mcu_cfg_t * p_lpmv2_s5d3_cfg);
static ssp_err_t check_standby_config(lpmv2_mcu_cfg_t * p_lpmv2_s5d3_cfg);
 ...
 **********************************************************************************************************************/
/* Tracks the module stop state for modules stopped during transition to standby */
static uint32_t g_module_state = 0U;

/* This array is used during R_LPMV2_LowPowerModeEnter to determine which modules to stop before entering standby. */
static const ssp_feature_t g_modules_to_stop[] =
{
 {
   {
    .id = SSP_IP_SCE,
...
ssp_err_t HW_LPMV2_MCUSpecificLowPowerCheck(lpmv2_cfg_t const * const p_cfg)
{
    ...
    if(LPMV2_LOW_POWER_MODE_SLEEP != p_cfg->low_power_mode)
    {
        ...
        lpmv2_mcu_cfg_t * p_lpmv2_s5d3_cfg = (lpmv2_mcu_cfg_t * )(p_cfg->p_extend);

        if(LPMV2_LOW_POWER_MODE_STANDBY_SNOOZE == p_cfg->low_power_mode)
        {
            err = check_snooze_config(p_lpmv2_s5d3_cfg->dtc_state_in_snooze,
                                                p_lpmv2_s5d3_cfg->snooze_request_source);
            LPMV2_ERROR_RETURN(SSP_SUCCESS == err, err);
        }
        else if(LPMV2_LOW_POWER_MODE_DEEP == p_cfg->low_power_mode)
        {
            err = check_deep_standby_config(p_lpmv2_s5d3_cfg);
            LPMV2_ERROR_RETURN(SSP_SUCCESS == err, err);
        }

        err = check_standby_config(p_lpmv2_s5d3_cfg);
        LPMV2_ERROR_RETURN(SSP_SUCCESS == err, err);
    }
 ...
 *
 * @param p_lpmv2_s5d3_cfg the MCU specific configuration
 *
 * @return SSP_SUCCESS              configuration is valid
 * @return SSP_ERR_INVALID_MODE     invalid power supply option for deep standby
 * @return SSP_ERR_INVALID_MODE     invalid IO port option for deep standby
...
static ssp_err_t check_deep_standby_config(lpmv2_mcu_cfg_t * p_lpmv2_s5d3_cfg)
{
    ...
    LPMV2_ERROR_RETURN(true ==
        check_deep_standby_cancel_sources(p_lpmv2_s5d3_cfg->deep_standby_cancel_source),
        SSP_ERR_INVALID_MODE);

    LPMV2_ERROR_RETURN(true ==
        check_deep_standby_cancel_edges(p_lpmv2_s5d3_cfg->deep_standby_cancel_edge),
        SSP_ERR_INVALID_MODE);

    LPMV2_ERROR_RETURN(true == check_deep_standby_cancel_sources_edges(
                            p_lpmv2_s5d3_cfg->deep_standby_cancel_source,
                            p_lpmv2_s5d3_cfg->deep_standby_cancel_edge),
                       SSP_ERR_INVALID_MODE);

    err = check_power_supply_state(p_lpmv2_s5d3_cfg->power_supply_state);
    LPMV2_ERROR_RETURN(SSP_SUCCESS == err, err);

    err = check_io_port_state(p_lpmv2_s5d3_cfg->io_port_state);
    LPMV2_ERROR_RETURN(SSP_SUCCESS == err, err);
...
static ssp_err_t check_standby_config(lpmv2_mcu_cfg_t * p_lpmv2_s5d3_cfg)
{
    LPMV2_ERROR_RETURN(true == check_wake_sources(p_lpmv2_s5d3_cfg->standby_wake_sources), SSP_ERR_INVALID_MODE);

    LPMV2_ERROR_RETURN(true == check_output_port_config(p_lpmv2_s5d3_cfg->output_port_enable), SSP_ERR_INVALID_MODE);
...
void HW_LPMV2_MCUSpecificConfigure(lpmv2_cfg_t const * const p_cfg)
{
       ...
       (NULL != p_cfg->p_extend))
    {
        ...
        if(LPMV2_LOW_POWER_MODE_DEEP == p_cfg->low_power_mode)
        {
            ...
            R_SYSTEM->DSPBYCR_b.DEEPCUT = DPSBYCR_DEEPCUT_MASK & p_lpmv2_s5d3_cfg->power_supply_state;

            HW_LPMV2_DPSIER0Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d3_cfg->deep_standby_cancel_source       )));
            HW_LPMV2_DPSIER1Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d3_cfg->deep_standby_cancel_source >> 8U )));
            HW_LPMV2_DPSIER2Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d3_cfg->deep_standby_cancel_source >> 16U)));
            HW_LPMV2_DPSIER3Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d3_cfg->deep_standby_cancel_source >> 24U)));

            HW_LPMV2_DPSIEGR0Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d3_cfg->deep_standby_cancel_edge       )));
            HW_LPMV2_DPSIEGR1Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d3_cfg->deep_standby_cancel_edge >> 8U )));
            HW_LPMV2_DPSIEGR2Set((uint8_t)(0xFFU & (uint8_t)(p_lpmv2_s5d3_cfg->deep_standby_cancel_edge >> 16U)));

            R_SYSTEM->DSPBYCR_b.IOKEEP = DPSBYCR_IOKEEP_MASK & p_lpmv2_s5d3_cfg->io_port_state;
        }
        else if(LPMV2_LOW_POWER_MODE_STANDBY_SNOOZE == p_cfg->low_power_mode)
        {
            HW_LPMV2_SnoozeConfigure(p_lpmv2_s5d3_cfg->dtc_state_in_snooze,
                                     p_lpmv2_s5d3_cfg->snooze_request_source,
                                     p_lpmv2_s5d3_cfg->snooze_end_sources,
                                     p_lpmv2_s5d3_cfg->snooze_cancel_sources);
            R_SYSTEM->SNZCR |= SNZCR_SNZE_MASK;
        }

        R_SYSTEM->SBYCR_b.OPE = SBYCR_OPE_MASK & p_lpmv2_s5d3_cfg->output_port_enable;

        HW_LPMV2_WUPENSet(p_lpmv2_s5d3_cfg->standby_wake_sources);
...
ssp_err_t HW_LPMV2_LowPowerModeEnter (void)
{
    ...
    /*LDRA_NOANALYSIS CMSIS related functions are not maintained by Renesas,
     *  so LDRA analysis is skipped for these files. */
    __WFI();
    /*LDRA_ANALYSIS */
    ...
    if(1U == R_SYSTEM->SBYCR_b.SSBY)
    {
        ...
        /* Restore stopped modules after resuming from standby */
        for (uint32_t i = 0U; i < (sizeof(g_modules_to_stop) / sizeof(g_modules_to_stop[0])); i++)
        {
            if (!(g_module_state & (1U << i)))
            {
                R_BSP_ModuleStart(&g_modules_to_stop[i]);
            }
        }
    }
...
static ssp_err_t pre_wfi_standby(void)
{
    bool modules_were_stopped = false;
    ssp_err_t ssp_err = SSP_SUCCESS;
    ...
    /* Stop high current modules before standby */
    g_module_state = 0U;
    for (uint32_t i = 0U; i < (sizeof(g_modules_to_stop) / sizeof(g_modules_to_stop[0])); i++)
    {
        bool module_stopped = false;
        R_BSP_ModuleStateGet(&g_modules_to_stop[i], &module_stopped);
        if (!module_stopped)
        {
            modules_were_stopped = true;
            R_BSP_ModuleStopAlways(&g_modules_to_stop[i]);
        }
        else
        {
            g_module_state |= (1U << i);
