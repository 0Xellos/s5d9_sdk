...
/*******************************************************************************************************************//**
 * Enable input noise filter for input signal on GTIOCA and GTIOCB pin.
 * @param   p_gpt_base       Pointer to base register of GPT channel.
 * @param   filter_set       Enable or disable the filter
 * @param   signal_pin       Specify which pin to use.
 **********************************************************************************************************************/
__STATIC_INLINE void HW_GPT_InputSignalNoiseFilterSet(GPT_BASE_PTR p_gpt_base, bool filter_set, gpt_input_capture_signal_t signal_pin )
{
  if(GPT_INPUT_CAPTURE_SIGNAL_PIN_GTIOCA == signal_pin)
  {
    p_gpt_base->GTIOR_b.NFAEN = filter_set;
  }
  else
    p_gpt_base->GTIOR_b.NFBEN = filter_set;
}

/*******************************************************************************************************************//**
 * Sets the sampling clock for input signal noise filter.
 * @param   p_gpt_base         Pointer to base register of GPT channel.
 * @param   filter_select      Specify the filter value
 * @param   signal_pin         Specify which pin to use.
 **********************************************************************************************************************/
__STATIC_INLINE void  HW_GPT_NoiseFilterSamplingClockSelect(GPT_BASE_PTR p_gpt_base, gpt_input_capture_signal_filter_t filter_select, gpt_input_capture_signal_t signal_pin )
{
   if(GPT_INPUT_CAPTURE_SIGNAL_PIN_GTIOCA == signal_pin)
   {
     p_gpt_base->GTIOR_b.NFCSA = filter_select;
   }
   else
     p_gpt_base->GTIOR_b.NFCSB = filter_select;
}
