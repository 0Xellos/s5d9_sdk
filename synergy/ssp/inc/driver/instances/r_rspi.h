...
#define RSPI_CODE_VERSION_MAJOR       (2U)
#define RSPI_CODE_VERSION_MINOR       (0U)
...
typedef struct st_spi_on_rspi_cfg
{
    ...
    rspi_byte_swap_t      byte_swap;      /**< Feature for byte swapping */
} spi_on_rspi_cfg_t;
...
typedef struct st_rspi_instance_ctrl
{
    ...
    bool              using_dtc;           /* State flag indicating if DTC is being used. */
    transfer_addr_mode_t tx_dtc_addr_mode; /* Addressing mode of opened TX transfer instance. */
    transfer_addr_mode_t rx_dtc_addr_mode; /* Addressing mode of opened RX transfer instance. */
    spi_operation_t   transfer_mode;       /* Transmit only, receive only, or transmit-receive. */
    uint32_t          rx_data;             /* temporary storage for transfer data */
    bsp_lock_t        resource_lock_tx_rx; /* Resource lock for transmission/reception */
    rspi_byte_swap_t  byte_swap;           /* Feature for byte swap */
    uint8_t           tx_handler_spdr_ha;  /* To access data register in half word when byte swap is enabled */
